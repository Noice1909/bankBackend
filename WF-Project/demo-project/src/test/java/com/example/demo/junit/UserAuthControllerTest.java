...
class UserAuthControllerTest {
    ...
    void testGetAllUserAuths() throws Exception {
        ...
        Customer customer2 = new Customer(2L, "Jack Torson", "jack@example.com", "9087654321", "123 Main St", "NY", "Regionland", "12345");
        UserAuth user1 = new UserAuth(1L, new Customer(), "user1", "password1");
        UserAuth user2 = new UserAuth(2L, new Customer(), "user2", "password2");
    ...
    void testUpdateUserAuthById() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/user-auth/1")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void testDeleteUserAuthById() throws Exception {
        when(userAuthRepository.findById(anyLong())).thenReturn(Optional.of(new UserAuth()));

        mockMvc.perform(delete("/api/v1/user-auth/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.UserAuth has been Deleted").value(true));
    }
